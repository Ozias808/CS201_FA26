...
Answer: sp_attack value is skewed to the right. this means that their is unusually high values pulling it more to one side.
...
Answer: the bars in a histogram touch but the bars in a bar chart do not touch. bar chart is used for categorical data.
...
```{r}
#| label: scatter plot # geom point is scatter plot

ggplot(pokedata, aes(x = sp_atk, y = sp_def)) + 
  geom_point() +
  geom_smooth(method="lm", se=FALSE) # line average
```
